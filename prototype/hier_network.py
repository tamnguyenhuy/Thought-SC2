...
class HierNetwork(object):
    ...
    def _create_graph(self):
        ...
        with tf.device(tf.compat.v1.train.replica_device_setter(worker_device=worker_device, cluster=self.cluster)):
            self.results_sum = tf.compat.v1.get_variable(trainable=False, name="results_sum", shape=[], initializer=tf.compat.v1.zeros_initializer)
            self.game_num = tf.compat.v1.get_variable(trainable=False, name="game_num", shape=[], initializer=tf.compat.v1.zeros_initializer)

            self.global_steps = tf.compat.v1.get_variable(trainable=False, name="global_steps", shape=[], initializer=tf.compat.v1.zeros_initializer)
